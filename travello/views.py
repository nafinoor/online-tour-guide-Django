...
def pessanger_detail_def(request, city_name):
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            ...
            if temp_date < date1:
                messages.info(request, 'You have entered a previous date')
                return redirect('index')
            ...
            for i in range(0, formset.total_form_count()):
                ...
                t = pessanger_detail(Trip_same_id=pipo_id,first_name=form.cleaned_data['first_name'], last_name=form.cleaned_data['last_name'],
                                     ...
                                     Trip_date=temp_date,payment=price,username=usernameget,city=city,country=country)
                t.save()
            ...
            request.session['no_of_person'] = no_of_person
            request.session['price1'] = price1
            request.session['GST'] = GST
            request.session['country'] = country

            return render(request,'payment.html', {'no_of_person': no_of_person,
                                                   'price1': price1, 'GST': GST, 'final_total': final_total,'city': city, 'country': country })
    else:
...
def net_payment(request):
    ...
    try:
        ...
        if int(balance) >= int(request.session['pay_amount']):
            ...
            city = request.session['city']
            country = request.session['country']
            no_of_person = request.session['no_of_person']
            price1 = request.session['price1']
            GST = request.session['GST']
            date1 = datetime.now().date()
            price = request.session['price']
            return render(request, 'confirmetion_page.html', {'no_of_person': no_of_person, 'price': price,
                                                   'price1': price1, 'GST': GST, 'final_total': amt,'city': city, 'usernameget': usernameget, 'date': date1, 'country': country, })
        else:
